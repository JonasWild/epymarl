    ...
    def log_heatmaps(self, heatmaps: [MyHeatmap]):
        if self.use_tb and heatmaps is not None:
            for heatmap in heatmaps:
                ...
                # Calculate the relative width ratios for the heatmap and colorbar
                heatmap_width_ratio = heatmap_aspect * 7  # Adjust this value to control the heatmap width
                colorbar_width_ratio = 1  # Adjust this value for colorbar width

                # Create a GridSpec with the calculated width ratios
                ...
                if isinstance(heatmap, HeatSC2map):
                    ...
                    ax_heatmap = plt.subplot(gs[0])
                    bg_plot = ax_heatmap.imshow(background_image, extent=[0, heatmap_width, 0, heatmap_height], alpha=0.5)
