...
class ForagingEnv(Env):
    ...
    def __init__(
        ...
        self.coupled_collected_food_episode = 0
        self.loading_failed_in_episode = 0
    ...
    def spawn_food(self, max_food):
        ...
        # CUSTOMIZED
        # min_level = np.min([player.level for player in self.players])

        min_level = 100000
        max_level = 0
        for i in range(len(self.players)):
            for j in range(i + 1, len(self.players)):
                sum_of_levels = self.players[i].level + self.players[j].level
                max_level = max(max_level, sum_of_levels)
                min_level = min(min_level, sum_of_levels)
        ...
        for player in self.players:
            ...
            while attempts < 1000:
                ...
                if self._is_empty_location(row, col):
                    # level = self.np_random.randint(1, max_player_level + 1)
    ...
    def step(self, actions):
        ...
        for player, action in zip(self.players, actions):
            min_lvl = min(min_lvl, player.level)
            max_lvl = max(max_lvl, player.level)
            ...
            if action == Action.NONE:
                ...
                collisions[player.position].append(player)
                self.loading_players.add(player)
        ...
        while self.loading_players:
            # find adjacent food
            player = self.loading_players.pop()
            frow, fcol = self.adjacent_food_location(*player.position)
            food = self.field[frow, fcol]

            adj_players = self.adjacent_players(frow, fcol)
            adj_players = [
                p for p in adj_players if p in self.loading_players or p is player
            ]

            adj_player_level = sum([a.level for a in adj_players])

            self.loading_players = self.loading_players - set(adj_players)

            if adj_player_level < food:
                # failed to load
                for a in adj_players:
                    a.reward -= self.penalty
                    self.loading_failed_in_episode += 1
                continue

            # else the food was loaded and each player scores points
            for a in adj_players:
                if len(adj_players) > 1:
                    self.coupled_collected_food_episode += 1

                balancing = ((max_lvl + 1) - a.level)**2
                self.level_balanced_reward += (a.reward * balancing) - a.reward
            ...
            min_lvl_player = min(adj_players, key=lambda adj: adj.level)
            min_lvl_player.reward = food
            min_lvl_player.level += food
